 ...
 * Excel视图（多个sheet页）
 */
public class MulExcelView extends AbstractExcelView {

    @Override
    protected void buildExcelDocument(Map<String, Object> model, HSSFWorkbook workbook, HttpServletRequest request, HttpServletResponse response) throws Exception {
        String[] sheets = (String[]) model.get("sheets");
        ...
        for (int i = 0, l = sheets.length; i < l; i++) {
            HSSFSheet sheet = workbook.createSheet(sheets[i]);  // 创建sheet页
            sheet.setDefaultColumnWidth(12);                    // 设置默认列宽

            // 创建表头行
            String[] head = heads[i];
            this.createHeadRow(sheet, head);

            // 写入数据行
            String[] column = columns[i];
            String data = datas[i];
            List dataList = JsonUtils.toObject(data, List.class);
            this.createDataRow(sheet, column, dataList);
        }
