...
<div class="pageMain">
    ...
    <div class="jigouDivHead">
        ...
        <c:if test="${doctor.auditState == 2}">
            <a class="buyA" href="${path}/admin/doctor/${doctor.id}/update"><spring:message code="common.btn.edit"/></a>
        </c:if>
        ...
        <div class="jigouDivHeadBody">
            <table class="userMsgTable">
                ...
                <tr>
                    <td><spring:message code="admin.doctor.form.address"/>:</td><th colspan="5">${doctor.address}</th>
                </tr>
            </table>
        </div>
        <%-- 认证图标 --%>
                ...
                <ul class="gongneng xw_fujian" id='panel1' style="margin:0 auto;">
                    <c:if test="${!empty doctor.doctorImages}">
                        <c:forEach var="doctorImage" items="${doctor.doctorImages}">
                            <li>
                                <img src="${basePath}/file?path=${doctorImage.thumbnail}" width="250" height="180" alt="" source="${path}/file?path=${doctorImage.source}"/>
                            </li>
                        </c:forEach>
                    </c:if>
                </ul>
