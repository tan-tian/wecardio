...
<head>
    ...
    <title><spring:message code="admin.doctor.todo.title"/></title>
    <link rel="stylesheet" type="text/css" href="${path}/resources/css/dingdanlist_ctr.css"/>
    <script type="text/javascript" src="${path}/resources/admin/doctor/js/todo.js"></script>
</head>
<body>
<div class="pageMain">
    <div class="addressBar">
        ...
        <span class="normal"><spring:message code="admin.bread.doctor.todo"/></span>
    ...
    <div class="objBoxCont" style="margin-bottom:10px;">
        <div class="searchFormMain" style="margin-bottom:0px;">
            <form id="qform" action="" method="post">
                ...
                <div class="crsearch">
                    <input type="text" name="name" style="width:200px;" placeholder="<spring:message code="admin.doctor.placeholder.search"/>" />
                    <a class="queryBtn"></a>
                </div>
...
<textarea id="template" rows="0" cols="0" style="display: none">
    ...
    <div class="sgListLiMain xw_sgListLiMain" did="{$T.record.id}" guid="{$T.record.guid}">
        ...
        <div class="sgListLiMainCenter xw_sgListLiMainCenter">
            <div class="sgListHead"><h1>{$T.record.name}</h1></div>
            <ul class="sgMsg">
                <li><span><spring:message code="admin.doctor.list.org"/>:</span><a>{$T.record.orgName}</a></li>
                <li><span><spring:message code="admin.doctor.list.org.type"/>:</span><a>{$T.record.orgTypeName}</a></li>
                <li><span><spring:message code="admin.doctor.list.org.contact"/>:</span><a>{$T.record.orgContact}</a></li>
                <li><span><spring:message code="admin.doctor.list.applyDate"/>:</span><a>{$T.record.createDate}</a></li>
            </ul>
        </div>
    </div>
    {#/for}
