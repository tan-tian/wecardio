...
<head>
    ...
    <title><spring:message code="org.doctor.view.title"/></title>
    ...
    <script type="text/javascript" src="${path}/resources/org/doctor/js/view.js"></script>
</head>
<body>
<div class="pageMain">
    <div class="addressBar">
        ...
        <span class="arrowText">&gt;</span>
        <span class="normal"><spring:message code="org.bread.doctor.view"/></span>
    ...
    <div class="jigouDivHead">
        ...
        <%-- 申请认证按钮 --%>
        <c:if test="${doctor.auditState == 0}">
        <a class="buyA" href="${path}/org/doctor/${doctor.id}/edit"><spring:message code="common.btn.doctor.authentication"/></a>
        </c:if>
        <%-- 编辑按钮 --%>
        <c:if test="${doctor.auditState == 2}">
            <a class="buyA" href="${path}/org/doctor/${doctor.id}/update"><spring:message code="common.btn.edit"/></a>
        </c:if>
        ...
        <div class="jigouDivHeadBody">
            <table class="userMsgTable">
                <tr>
                    <td><spring:message code="org.doctor.form.account"/></td><th>${doctor.account}</th>
                    <td><spring:message code="org.doctor.form.sex"/>:</td><th><spring:message code="common.enum.sex.${doctor.sex}"/> </th>
                    <td><spring:message code="org.doctor.form.ic"/>:</td><th>${doctor.ic}</th>
                </tr>
                <tr>
                    <fmt:formatDate value="${doctor.doctorBirthday}" var="doctorBirthday" pattern="yyyy-MM-dd"/>
                    <td><spring:message code="org.doctor.form.birthday"/>:</td><th>${doctorBirthday}</th>
                    <td><spring:message code="org.doctor.form.mobile"/>:</td><th>${doctor.mobile}</th>
                    <td><spring:message code="org.doctor.form.email"/>:</td><th>${doctor.email}</th>
                </tr>
                <tr>
                    <td><spring:message code="org.doctor.form.address"/>:</td><th colspan="5">${doctor.address}</th>
                </tr>
            </table>
        </div>
        <%-- 认证图标 --%>
    ...
    <div class="detailContent">
        <h1><spring:message code="org.doctor.form.intro"/>:</h1>
