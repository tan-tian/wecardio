...
<head>
    ...
    <title><spring:message code="patient.record.view.title"/></title>
    <link rel="stylesheet" type="text/css" href="${path}/resources/css/dingdandetail_style.css"/>
    <script type="text/javascript" src="${path}/resources/patient/record/js/view.js"></script>
</head>
<body>
<div class="pageMain">
    <div class="addressBar">
        ...
        <span class="arrowText">&gt;</span>
        <span class="normal"><spring:message code="patient.bread.record.view"/></span>
                ...
                <h2 style="padding-top:45px;">
                    <c:choose>
                        <c:when test="${record.isCommit}">
                            <spring:message code="patient.record.list.state.done"/>
                        </c:when>
                        <c:otherwise>
                            <spring:message code="patient.record.list.state.todo"/>
                        </c:otherwise>
                    </c:choose>
                </h2>
                <h3>[<spring:message code="patient.record.view.state"/>]</h3>
            </div>
        ...
        <div class="projectDetailHeadBody">
            <div class="projetListConFoot">
                <table class="userMsgTable">
                    <tr>
                        <td><spring:message code="patient.record.view.serviceType"/>:</td><th>${record.serviceTypeName}</th>
                        <td><spring:message code="patient.record.view.doctor"/>:</td><th>${record.doctorName}</th>
                        <td><spring:message code="patient.record.view.ext"/>:</td>
                        <th>
                            <c:choose>
                    ...
                    <tr>
                        <fmt:formatDate value="${record.testDate}" var="testDate" pattern="yyyy-MM-dd HH:mm:ss" />
                        <td><spring:message code="patient.record.view.tested"/>:</td><th>${testDate}</th>
                        <fmt:formatDate value="${record.createDate}" var="createDate" pattern="yyyy-MM-dd" />
                        <td><spring:message code="patient.record.view.created"/>:</td><th>${createDate}</th>
                        <td></td>
                        <th id="flag"></th>
                    </tr>
                    <tr>
                        <td><spring:message code="patient.record.view.symptom"/>:</td><th>${record.condition}</th>
                        <td><spring:message code="patient.record.view.org"/>:</td><th>${record.orgName}</th>
                    </tr>
                    <tr>
                        <td><spring:message code="patient.record.view.findings"/>:</td><th  colspan="3">${record.findings}</th>
                    </tr>
                </table>
            </div>
        </div>
    ...
    <div class="userMain" style="margin-bottom:10px;">
        <div class="userDetailCont" style="padding-top:0px;">
            ...
            <div class="userDetailContHead">
                <h1 class="userDetailConthText"><spring:message code="patient.record.view.pdf"/></h1>
