...
seajs.use(['$', 'validate', 'msgBox', 'base64', 'rsa', 'cookie'], function ($, validate, msgBox, base64, rsa, cookie) {
    ...
    $(".mainUlLink2").on('click', function() {
        msgBox.exWindow.open({
            ...
            width : '430px',
            height : '325px',
            url : path + '/patient/password/find'
        });
    });

    if ($.cookie("patient.username") != null) {
        $('#username').val($.cookie("patient.username"));
        $('#passwrd').focus();
    } else {
        $('#username').focus();
    }
    ...
    $('#submitBtn').on('click', function() {
        ...
        if ($form.valid()) {
            ...
            $.cookie("patient.username", $username.val(), {expires: 7 * 24 * 60 * 60});
            ...
            $.post($form.attr('action'), $form.serialize(), function (data) {
                if (data.type == 'success') {
                    window.location.href = path + '/patient';
