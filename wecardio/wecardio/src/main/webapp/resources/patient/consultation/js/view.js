...
seajs.use(['msgBox'], function (msgBox) {
    $(document).ready(function() {
        ...
        $.post(path + '/patient/consultation/pdf', { id : $('#cid').val() }, function(data) {
        ...
        $('#evaluateBtn').on('click', function () {
            msgBox.exWindow.open({
                ...
                width : '700px',
                height : '300px',
                url : path + '/patient/consultation/' + $('#cid').val() + '/evaluate'
            });
        });

        $('#downloadBtn').on('click', function () {
            window.location.href = path + '/patient/consultation/pdf/download?cid=' + $('#cid').val();
        });
    });
