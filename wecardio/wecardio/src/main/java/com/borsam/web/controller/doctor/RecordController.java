 ...
 */
@Controller("doctorRecorderController")
@RequestMapping(value = "/doctor/record")
public class RecordController extends BaseController {
    ...
    public Page<Record> page(String serviceName, Long serviceType, Date startDate, Date endDate, Integer[] state,
                             Integer[] flag, Integer[] stoplight, Pageable pageable) {
        ...
        if (((doctorProfile.getRoles() & 2)== 2) ) {
            ...
            // 只查绑定当前机构绑的VIP患者的记录
            filters.add(Filter.eq("patient.org", doctorAccount.getDoctorProfile().getOrg()));
        }
        ...
        if (flag != null && flag.length > 0) {
            filters.add(Filter.in("flag", flag));
        }
